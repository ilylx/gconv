...
const (
	...
	gDefaultQueueSize = 10000
	// Max batch size per-fetching from list.
	gDefaultMaxBatchSize = 10
)
...
func New(limit ...int) *Queue {
	...
	if len(limit) > 0 && limit[0] > 0 {
		...
		q.C = make(chan interface{}, gDefaultQueueSize)
	...
	for !q.closed.Val() {
		...
		for !q.closed.Val() {
			if length := q.list.Len(); length > 0 {
				if length > gDefaultMaxBatchSize {
					length = gDefaultMaxBatchSize
				}
...
func (q *Queue) Push(v interface{}) {
	if q.limit > 0 {
		...
		q.list.PushBack(v)
		if len(q.events) < gDefaultQueueSize {
			q.events <- struct{}{}
		}
	}
}
...
func (q *Queue) Close() {
	...
	for i := 0; i < gDefaultMaxBatchSize; i++ {
		q.Pop()
	}
}
