...
func New(slot int, interval time.Duration, level ...int) *Timer {
	...
	for i := 0; i < length; i++ {
		if i > 0 {
			...
			t.wheels[i] = w
			t.wheels[i-1].addEntry(n, w.proceed, false, gDefaultTimes, StatusReady)
		} else {
...
func (t *Timer) Add(interval time.Duration, job JobFunc) *Entry {
	return t.doAddEntry(interval, job, false, gDefaultTimes, StatusReady)
}
...
func (t *Timer) AddSingleton(interval time.Duration, job JobFunc) *Entry {
	return t.doAddEntry(interval, job, true, gDefaultTimes, StatusReady)
}
